                            ...
                            <div class="modal-dialog modal-md" role="document">
                                <div class="modal-content-view">
                                    ...
                                    <div class="modal-footer">
                                        <p>Can You Attend?</p>
                                        <button type="button" class="btn btn-primary" id="deleteEventYes" data-dismiss="modal">Yes</button>
                                        <button type="button" class="btn btn-danger" id="deleteEventNo">No</button>
                                    </div>
                                </div>
                            </div>
                        </div>
...
<script>
			...
			success: function (response) {
				...
				var calendar = $('#calendar').fullCalendar({
                    ...
                    eventClick: function(calEvent, jsEvent, view) {
                        ...
                        $('#eventDetailsModal').data('event-id', calEvent.event_id);
                        ...
                        // Display only the first 7 user emails associated with the event
                        var userEmails = calEvent.user_emails;
                        var userEmailsHtml = '';
                        if (userEmails && userEmails.length > 0) {
                            userEmailsHtml += '<ul>';
                            var maxDisplay = Math.min(5, userEmails.length); // Maximum number of emails to display
                            for (var i = 0; i < maxDisplay; i++) {
                                userEmailsHtml += '<li>' + userEmails[i] + '</li>';
                            }
                            userEmailsHtml += '</ul>';
                            
                            // If there are more than 7 emails, add a "View All" button
                        ...
                        $('.view-all-emails').click(function() {
                            ...
                            var modalHtml = '<div class="modal fade" id="viewAllEmailsModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">';
                            modalHtml += '<div class="modal-dialog" role="document">';
                            modalHtml += '<div class="modal-contents" style="background: white;">';
                            modalHtml += '<div class="modal-header">';
                            modalHtml += '<h5 class="modal-title" id="exampleModalLabel">All Participants</h5>';
                            modalHtml += '<button type="button" class="close" data-dismiss="modal" aria-label="Close">';
                            modalHtml += '<span aria-hidden="true">&times;</span>';
                            modalHtml += '</button>';
                            modalHtml += '</div>';
                            modalHtml += '<div class="modal-body">';
                            modalHtml += allUserEmailsHtml;
                            modalHtml += '</div>';
    ...
    $(document).on('click', '#deleteEventNo', function() {
        var eventId = $('#eventDetailsModal').data('event-id');

        // Call a function to delete the event from the database
        deleteEvent(eventId);
    });
    ...
    function deleteEvent(eventId) {
        ...
        $.ajax({
            ...
            success: function(response) {
                // Reload the calendar after successful deletion
                $('#calendar').fullCalendar('refetchEvents');

                // Reload the page after deleting the event
                location.reload();
            },
